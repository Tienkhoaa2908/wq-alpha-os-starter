...
It does not invent arbitrary formulas.  Instead it searches the already-audited
space:
...
confirmation and relation templates are better introduced only after evidence
exists, because random pair search would explode the trial space.
"""
...
from .motifs import motif_fingerprint, novelty_diagnostics
...
AUTONOMOUS_VERSION = "deterministic-semantic-search-v1"
...
def _active_used_fields(connection: sqlite3.Connection) -> set[str]:
    ...
    rows = connection.execute(
        "SELECT field_names_json FROM alpha_artifacts WHERE status<>'legacy_unverified'"
    ).fetchall()
    for row in rows:
        try:
    ...
    *,
    ...
    for row in _profile_rows(connection):
        ...
        for template in templates:
            ...
            # Quality prior is deliberately simple and auditable.  Empirical
            # performance only enters later, after BRAIN evidence exists.
            base = (
                2.0 * confidence
                + 0.8 * (coverage / 100.0)
            ...
            if template.id in {"vector_event_novelty", "information_staleness", "extremum_recency"}:
                base += 0.20  # underexplored semantics receive a small exploration prior
...
def select_diverse(pool: list[SearchCandidate], count: int = 6) -> list[SearchCandidate]:
    """Greedily maximize candidate quality plus new theme/dataset information."""
    ...
    while remaining and len(selected) < count:
        ...
        for item in remaining:
            ...
            if theme_counts[item.theme] == 0:
                score += 1.40
            else:
                score -= 0.55 * theme_counts[item.theme]
            if dataset_counts[item.dataset] == 0:
                score += 1.10
            else:
                score -= 0.70 * dataset_counts[item.dataset]
...
def materialize_autonomous_breadth(
    ...
    *,
    ...
    No network request is made here: no Gemini, Groq, OpenRouter or BRAIN.
    """
