...
- SQLite trong `data/db/` và bằng chứng trong `data/evidence/` là nguồn sự thật thực nghiệm cục bộ; các tệp trạng thái trong `docs/` là bản rút gọn an toàn để đồng bộ qua GitHub.
...
4. Xuất snapshot cục bộ an toàn bằng `python .\scripts\export_research_state.py` để cập nhật:
   - `docs/TRANG_THAI_HIEN_TAI.md`
   - `docs/generated/research_state.json`
5. Commit và push branch hiện tại lên GitHub.
...
Script này chạy test, rebuild knowledge, xuất snapshot, stage các file source-controlled, commit và push branch hiện tại. Không stage `.env`, SQLite, evidence hoặc exports runtime.
